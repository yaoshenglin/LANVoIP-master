...
- (void)configAudio
{
    ...
    [self.session setCategory:AVAudioSessionCategoryPlayAndRecord error:&error];//播放和记录
    handleError(error);
    //route变化监听
    ...
    [self.session setPreferredIOBufferDuration:0.005 error:&error];//设置首选I/O缓冲时间(每一帧数据间隔)
    handleError(error);
    [self.session setPreferredSampleRate:kSmaple error:&error];//采样率
    handleError(error);
    
    //[self.session overrideOutputAudioPort:AVAudioSessionPortOverrideSpeaker error:&error];
    //handleError(error);
    
    [self.session setActive:YES error:&error];//设置Mode(激活)
    handleError(error);
    ...
    targetDes.mFormatID = kAudioFormatMPEG4AAC;//AAC采样
    targetDes.mSampleRate = kSmaple;//采样率
    targetDes.mChannelsPerFrame = sourceDes.mChannelsPerFrame;//1单声道，2立体声
    
    UInt32 size = sizeof(targetDes);
    CheckError(AudioFormatGetProperty(kAudioFormatProperty_FormatInfo, 0, NULL, &size, &targetDes), "couldnt create target data format");


    //选择软件编码
    AudioClassDescription audioClassDes;
    audioClassDes.mType = 1;
    ...
    //new convertRef
    CheckError(AudioConverterNewSpecific(&sourceDes, &targetDes, 2, &audioClassDes, &_encodeConvertRef), "cant new convertRef");
    ...
    CheckError( AudioQueueSetParameter(_playQueue, kAudioQueueParam_Volume, 1.0), "cant set audio queue gain");
    ...
    [self performSelectorInBackground:@selector(playData) withObject:nil];//Audio Queue播放音频数据
}
...
- (void)dataInit
{
    ...
    _encodeConvertRef = malloc(sizeof(AudioConverterRef)*2);
    
}

//采集音频数据的输入回调
OSStatus inputRenderTone(void *inRefCon,
